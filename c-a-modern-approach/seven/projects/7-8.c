...
int main(void) {
	...
	//ALTERED: ask user for 12-hour time (instead of 24); space at end of scanf accounts for possible whitespace before amPm)
	printf("Please Enter 12-hour time (AM/PM): ");
	scanf("%2d:%02d ", &hInputTime, &mInputTime);

	// NEW: Determine whether AM or PM. Produces an A or P
	if ((ch = getchar()) >= 65 && ch <= 122) {
		amPm = toupper(ch);
	}		

	// Convert time to minutes
	// NEW Modulo 12 added so 12:30am equals 30min, not 750 min
	if (amPm == 'A') {

		inputMinutes = ((hInputTime % 12) * 60) + mInputTime;
	} else {
		inputMinutes = (((hInputTime % 12) + 12) * 60) + mInputTime;
	}

	// Used for debugging. uncomment to confirm times are correct
	//printf("Current value of amPm: %c\n", amPm);	
	...
	if (hInputTime > 12 || hInputTime < 1 || mInputTime > 60 || mInputTime < 0) {
			printf("Error: Out of Range. Allowable values: 1-12, 0-60, AM-PM\n");
			return 1;	
	}
	//check times
	if ((inputMinutes <= 480) || inputMinutes >= 1305) {
